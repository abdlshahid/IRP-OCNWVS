...
print("\n" + "=" * 70)
print("STEP 2: Extracting inputs S_f, a1, b1")
print("=" * 70)
...
# ============================================================================
# 3️⃣ RECONSTRUCT BASELINE SPECTRUM
# ============================================================================

print("\n" + "=" * 70)
print("STEP 3: Reconstructing baseline spectrum using directional moments")
print("=" * 70)
...
# Baseline spectrum: S_baseline(f, θ) = S_f(f) × D(f, θ)
# where D(f, θ) = 1/(2π) × (1 + 2·a1·cos(θ) + 2·b1·sin(θ))
...
for t in range(n_time):
    for f in range(n_freq):
        ...
        # S_baseline(f, θ) = S_f(f) × D(f, θ)
        S_baseline[t, f, :] = S_f_val * D

print(f"✓ Reconstructed baseline spectrum")
print(f"  S_baseline shape: {S_baseline.shape} (time, 30, 24)")
print(f"  S_baseline range: [{np.nanmin(S_baseline):.6e}, {np.nanmax(S_baseline):.6e}]")
...
S_baseline_clean = np.nan_to_num(S_baseline, nan=0.0)

# Relative L2 error per time-freq pair
# error = ||s_baseline - s_true|| / ||s_true||
epsilon = 1e-10  # avoid division by zero

norm_true = np.linalg.norm(y_true_clean.reshape(n_time, -1), axis=1)
norm_diff = np.linalg.norm((S_baseline_clean - y_true_clean).reshape(n_time, -1), axis=1)

relative_l2_error = norm_diff / (norm_true + epsilon)

# MSE per time step
mse_per_time = np.mean((S_baseline_clean - y_true_clean) ** 2, axis=(1, 2))

# Mean Absolute Error per time step
mae_per_time = np.mean(np.abs(S_baseline_clean - y_true_clean), axis=(1, 2))
...
print("\n" + "=" * 70)
print("BASELINE RECONSTRUCTION RESULTS")
print("=" * 70)
...
print(f"  D(f,θ) = 1/(2π) × (1 + 2a₁cos(θ) + 2b₁sin(θ))")
print(f"  S_baseline(f,θ) = S_f(f) × D(f,θ)")
